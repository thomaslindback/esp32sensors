...
// Change the device type if required. Do this after instantiation but before
// .begin
void SFE_MAX1704X::setDevice(sfe_max1704x_devices_e device) {
  ...
  switch (device) {
    ...
    case MAX1704X_MAX17049:
      _full_scale = 10.24;  // MAX17049 VCELL is 16-bit, 78.125uV/cell per LSB
                            // (i.e. 156.25uV per LSB)
      break;
...
bool SFE_MAX1704X::begin(uint8_t i2caddr, i2c_port_t master_port_num) {
  _master_port_num = master_port_num;
  _i2caddr = i2caddr;

  i2c_config_t conf = {.mode = I2C_MODE_MASTER,
                       .sda_io_num = MAX1704X_I2C_MASTER_SDA,
                       .scl_io_num = MAX1704X_I2C_MASTER_SCL,
                       .sda_pullup_en = GPIO_PULLUP_ENABLE,
                       .scl_pullup_en = GPIO_PULLUP_ENABLE,
                       .master{.clk_speed = MAX1704X_I2C_MASTER_FREQ_HZ},
                       .clk_flags = 0};
  ...
  err = i2c_driver_install(_master_port_num, conf.mode,
                           MAX1704X_I2C_MASTER_RX_BUF_DISABLE,
                           MAX1704X_I2C_MASTER_TX_BUF_DISABLE, 0);
  if (err != ESP_OK) {
...
esp_err_t SFE_MAX1704X::register_read(uint8_t reg_addr,
                                      uint8_t* data,
                                      size_t len) {
  return i2c_master_write_read_device(
      _master_port_num, _i2caddr, &reg_addr, 1, data, len,
      MAX1704X_I2C_MASTER_TIMEOUT_MS / portTICK_PERIOD_MS);
}
...
float SFE_MAX1704X::getVoltage() {
  ...
  if (_device <= MAX1704X_MAX17044) {
    ...
    // On the MAX17048/49: vCell is a 16-bit register where each bit
    // represents 78.125uV/cell per LSB i.e. 78.125uV per LSB on the MAX17048
...
// MAX1704x_STATUS_RI etc. will need to be shifted left by 8 bits to become
// aligned.
uint8_t SFE_MAX1704X::clearStatusRegBits(uint16_t mask) {
  ...
  return (
      write16(statusReg, MAX17048_STATUS));  // Write the contents back again
}
...
uint8_t SFE_MAX1704X::sleep() {
  ...
  if (configReg & MAX17043_CONFIG_SLEEP) {
    return MAX17043_GENERIC_ERROR;  // Already sleeping, do nothing but return
                                    // an error
  }
...
uint8_t SFE_MAX1704X::wake() {
  ...
  if (_device > MAX1704X_MAX17044) {
    // On the MAX17048, we should also clear the EnSleep bit in the MODE
    // register Strictly, this will clear the QuickStart bit too. Which is
    // probably a good thing, as I don't think we can do a read-modify-write?
...
esp_err_t SFE_MAX1704X::write16(uint16_t data, uint8_t address) {
  uint8_t msb, lsb;
  msb = (data & 0xFF00) >> 8;
  lsb = (data & 0x00FF);

  uint8_t write_buf[3] = {address, lsb, msb};

  return i2c_master_write_to_device(
      _master_port_num, _i2caddr, write_buf, sizeof(write_buf),
      MAX1704X_I2C_MASTER_TIMEOUT_MS / portTICK_PERIOD_MS);
}
...
uint16_t SFE_MAX1704X::read16(uint8_t address) {
  ...
  while ((success == false) && (retries > 0)) {
    uint8_t read_buff[2];
    esp_err_t err = register_read(address, read_buff, 2);
