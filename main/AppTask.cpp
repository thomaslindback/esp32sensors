...
namespace {
    constexpr auto APP_TASK_NAME = "APP";
    constexpr auto TAG = "app-task";
    constexpr auto APP_EVENT_QUEUE_SIZE = 10;
    constexpr auto APP_TASK_STACK_SIZE = 3072;
    constexpr auto GPIO_OUTPUT_IO_0 =  1;
    constexpr auto GPIO_OUPUT_PIN_SEL = 1ULL<<GPIO_OUTPUT_IO_0;
}
...
bme280::Adafruit_BME280 _sensor;

esp_err_t AppTask::sendData(float v, float p, float cr) {
    uint16_t MAX_HTTP_OUTPUT_BUFFER = 256;
    char local_response_buffer[MAX_HTTP_OUTPUT_BUFFER] = { 0 };
    char buffer[50];
    sprintf(buffer, "/hello/%.1f/%.1f/%.1f", v, p, cr);
...
void AppTask::OnTimerTimeout(TimerHandle_t xTimer) {
    ...
    //ESP_LOGI(TAG, "Change rate: %f", );
    sendData(v, p, fuel.getChangeRate());

}

esp_err_t AppTask::Init() {
    gpio_config_t io_conf = {
        .pin_bit_mask = GPIO_OUPUT_PIN_SEL,
